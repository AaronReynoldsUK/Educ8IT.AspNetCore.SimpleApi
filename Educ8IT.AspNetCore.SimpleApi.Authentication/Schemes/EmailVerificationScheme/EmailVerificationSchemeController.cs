    ...
    public class EmailVerificationSchemeController
    {
        ...
        public async Task<ActionResult> RequestAsync([FromBody] BearerTokenRequest bearerTokenRequest)
        {
            try
            {
                ...
                await _identityManagerService
                    .RemoveUserTokensAsync(__user.Id, false, EmailVerificationDefaults.RequestTokenName);
                ...
                var __charBytes = Encoding.UTF8.GetBytes(__verificationToken.Token);
                var __otc = await _identityManagerService.GenerateOTC(__charBytes);
        ...
        public async Task<ActionResult> VerifyAsync([FromBody] BearerTokenRequest bearerTokenRequest)
        {
            try
            {
                ...
                var __verificationToken = await _identityManagerService.GetTokenByTokenTypeForUserAsync(
                    __user.Id,
                    EmailVerificationDefaults.RequestTokenName);
                ...
                // Validate OTC
                var __charBytes = Encoding.UTF8.GetBytes(__verificationToken.Token);
                var __isVerified = _identityManagerService.VerifyOTC(
                    bearerTokenRequest.ConfirmationToken,
                    __verificationToken.ValidFrom,
                    __charBytes);
                ...
                // Remove previous tokens
                await _identityManagerService.RemoveUserTokensAsync(__user.Id, false, EmailVerificationDefaults.VerifyTokenName);

                // Remove request token too
                await _identityManagerService.RemoveUserTokensAsync(__user.Id, false, EmailVerificationDefaults.RequestTokenName);
